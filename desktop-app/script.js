...
/* 
    ...
    * hiddenAccessories stores hidden content in the 'hidden' property. It also
    saves hidden content in a local DB so they presist between application and page
*/
const hiddenAccessories = {
    ...
    init() {
        return new Promise((res, rej) => {
            let hiddenAccessoriesDB = indexedDB.open("testingDB3", 1);
            ...
            if (typeof node.id == 'string' && node.id.startsWith('chat-message')) {
                ...
                if (typeof node.className == 'string') {
                    if (node.className.split(' ').some(c => {
                        ...
                        chatMessages = Array.from(document.querySelectorAll('[class*="chatContent-"] [id*="chat-messages"]'));
                    }
                } 
            }
        });
